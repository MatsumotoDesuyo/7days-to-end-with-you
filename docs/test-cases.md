...
| UT-10 | Sentry 初期化 (instrument) | `SENTRY_DSN` 未設定なら init しない。設定時は dsn/environment/release + tracing で init する。traces は全量だが `/api/health`（probe）だけは送らない（tracesSampler、#52） | server `src/instrument.test.ts` | 済（#14） |
