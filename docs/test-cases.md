...
| ID | 対象機能 | 検証内容 | 実装 | 状態 |
|----|---------|---------|------|------|
| UT-01 | analyse-sentense (client) | N1: 25 文字周期・Z 非到達・実例復号 / N2: Z 入力の mod 26 / N3: 26 候補 / 小文字の観測挙動 | client `common/analyse-sentense.test.ts` | 済（カバレッジ 100%） |
| UT-02 | analyse-sentense (server) | UT-01 と同一（重複実装の一致保証。一本化は #6） | server `src/analyse-sentense.test.ts` | 済（カバレッジ 100%） |
